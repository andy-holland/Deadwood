...
public static void main(String[] args){
   ...
   private static int CreditPayout(Player player){
   return 0;
   }
   private static int MoneyPayout(Player player){
   return 0;
   }
   private static int BonusPayout(Player player){
   return 0;
   }
...
private boolean LeadingRole;
private boolean hasmoved = true; //why is this initalized to true?
   //constructor
   private Player(){
      instance += 1;
      PlayerNum = instance;
   ...
   public String ClaimRole(){//changes playerpart
      Scanner read = new Scanner(System.in);
      ...
      for(int i = 0; i < 4; i++){
         if(optionsB[i].ReturnTaken() == false && optionsB != null){
            System.out.println(optionsB[i].ReturnName());
         }
      }
      for(int i = 0; i < 3; i++){
         if(optionsS[i].ReturnTaken() == false && optionsS[i] != null){
            System.out.println(optionsS[i].ReturnName());
         }
      }
